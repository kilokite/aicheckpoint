...
列表左侧的轨迹记录每个仓库最近由 Checkpoint 创建或恢复到的快照指针。绿色节点是指针及其创建来源；虚线和“已跳过”表示不在当前创建轨迹上的快照，仍可恢复。红色箭头只展示最近一次恢复跳转，蓝色箭头展示恢复后创建的新快照如何从原快照分叉；同时出现的轨迹各占一条车道。拖动轨迹与列表之间的分隔线可调整宽度比例。选中列表行仅查看详情，不移动指针。

轨迹是操作记录，不会实时检测外部编辑或 Git 操作后的工作区内容，也不代表 Git 分支。旧版快照列表会原样保留，但无法准确推断历史指针和父子关系，因此从下一次创建或恢复开始记录；删除指针快照会清除指针，不会猜测替代位置。
...
选中快照后可以进入深色 Diff 页面。页面优先查看“创建来源 → 此快照”；旧快照没有来源记录时默认查看“上一个检查点 → 此快照”
之间的文件变化，也可以切换到“当前工作区 → 此快照”查看还原影响。最早的
快照没有上一个检查点时会自动使用当前工作区。Diff 支持新增、修改、删除、
重命名、二进制文件提示和逐文件文本差异，不会修改真实暂存区或工作区。
